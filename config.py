...
# 0.6 — то, что Qwen рекомендует для thinking-режима, и заодно лечит прежнюю
# крайность. На 0.2 модель шла в самый безопасный ответ: на задаче «найди
# отклонения» безопасный ответ — «отклонений нет», и она его и выдавала.
VIDEO_ANALYSIS_TEMPERATURE = float(os.getenv("VIDEO_ANALYSIS_TEMPERATURE", "0.6"))
...
# То есть и время, и деньги едут за длиной ролика почти линейно — причём время
# больнее: 153 секунды человек ждёт с телефоном в зале.
#
# Двадцать секунд — потолок, за которым для разбора техники ничего не
# добавляется. Паттерн виден на трёх повторах, а восьмой повтор не сообщает
# ничего, кроме усталости. Живые ролики в этот лимит укладываются: разобранные
# тройка тяги и четвёрка приседа — 10 и 16 секунд.
#
