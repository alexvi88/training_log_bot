...
LOCALIZED: list[str] = [
    ...
    # Политика конфиденциальности и условия (/privacy, /terms): текст — в
    # legal/<страница>.<язык>.html отдельными файлами на каждый язык, в самом
    # модуле литералов нет, язык — из ?lang или Accept-Language.
    "legal_pages.py",
    ...
    "api_v1_ai.py",
    # Откат сделанного тренером — общий для кнопки в боте и POST /ai/undo:
    # весь текст, который он возвращает человеку, — это ai.screen.undo.* через
    # i18n.t, своих литералов у модуля нет.
    "ai_undo.py",
    # Пересчёт кэшей FSM при смене кг↔lb — общий для кнопки бота и PATCH
    # /v1/settings. Текста не держит вовсе: только числа в состоянии диалога.
    "fsm_unit_rescale.py",
...
ALLOWED_CYRILLIC: dict[str, set[str]] = {
    ...
    "handlers/csv_import.py": {
        ...
        # _KG_UNIT_RU — «кг» в заголовке колонки веса («Вес (кг)»): единица
        # чужого файла, по которой _weight_factor решает, пересчитывать ли вес
        # в фунты аккаунта. Сравнение с вводом, не текст на экране.
        "кг",
    },
    ...
    "main.py": {
        ...
        "Проверочный пуш на iOS (админ)",
        "Что делают пользователи (админ)",
        "Воронка по источникам (админ)",
        "Сбои iOS-приложения по версиям (админ)",
...
NEVER_LOCALIZED: dict[str, str] = {
    ...
    "body_limit.py": (
        "ASGI-мидлварь потолка на тело запроса: наружу уходит только машинный "
    ...
    "exercise_photos.py": (
        "Хранение своих фото упражнений: имена файлов, путь на диске и "
        "журнальные записи для разработчика — человеку этот модуль ничего "
        "не показывает"
    ),
    "chat_attachments.py": (
        "Хранение вложений к истории чата с AI-тренером (фото/кадр-превью "
        "видео): имена файлов, путь на диске и журнальные записи для "
        "разработчика — человеку этот модуль ничего не показывает"
    ),
    "api_v1_media.py": (
    ...
    "api_v1_support.py": (
        "REST-переписка с поддержкой: наружу — реплики (их текст ввели атлет и "
        "админ, это данные, а не каталог) и машинные коды; банер атлету "
        "собирается через i18n.t_in по users.lang"
    ),
    "api_v1_funnel.py": (
        "анонимная воронка до входа: принимает машинный код шага и отдаёт "
        "{\"recorded\": bool}, человеческого текста у ручки нет"
    ),
    "api_v1_diagnostics.py": (
        "REST-приём отчётов о сбоях iOS (MetricKit): наружу уходят только "
        "машинные коды, сам отчёт — JSON от Apple, текста для атлета нет"
    ),
    "api_v1_import.py": (
    ...
    "review_demo.py": (
        "демо-аккаунт App Review: кириллица — только канонические имена "
        "шаблонов каталога (идентичность для поиска шаблона, seed_data), "
        "показываемое имя локализует fork_exercise_from_template по users.lang"
    ),
    "apple_signin.py": (
