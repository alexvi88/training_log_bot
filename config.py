...
#
# Цифровое внутри бота Telegram разрешает продавать только за звёзды, поэтому
# провайдера платежей тут нет вовсе: валюта XTR, provider_token пустой.
# Разбор экономики, рисков и почему платит именно AI-тренер — MONETIZATION.md.
#
# Дневник, история, графики, экспорт и MCP остаются бесплатными навсегда: они и
# есть доверие к продукту, а платит тот, кому уже ценно. Под платой — только
# AI-тренер, который на каждом вопросе реально жжёт деньги провайдера
# (см. cost_events и LLM_COSTS.md).

# Аварийный выключатель на весь раздел: с ним экран оплаты и кнопки исчезают, а
# уже купленный доступ продолжает действовать до своей даты — выключение
# продажи не должно отбирать оплаченное.
STARS_PAYMENTS_ENABLED = os.getenv("STARS_PAYMENTS_ENABLED", "true").lower() not in (
    "false", "0", "no",
)

# Сколько вопросов тренеру бесплатны в календарном месяце пользователя. Это и
# есть пейволл: дневной лимит ниже никуда не делся и по-прежнему защищает от
# разгона за один день, а месячный — та самая граница, за которой начинается
# подписка.
#
# Шестьдесят — примерно два вопроса в день: столько человек задаёт, когда
# тренер ему правда нужен, но ещё не «вместо гугла». По живым логам в этот
# потолок упирается меньшинство — ровно то, которому продукт уже ценен.
AI_QUESTION_MONTHLY_FREE = int(os.getenv("AI_QUESTION_MONTHLY_FREE", "60"))

# Дневной потолок для оплаченного доступа. Месячного у него нет, но полностью
# безлимитным он не становится: за деньги мы обещаем тренера без оглядки на
# счётчик, а не право прогнать через модель тысячу вопросов за ночь.
AI_QUESTION_DAILY_LIMIT_PRO = int(os.getenv("AI_QUESTION_DAILY_LIMIT_PRO", "150"))

# Разборов видео в день с оплаченным доступом (без него — AI_VIDEO_DAILY_LIMIT).
AI_VIDEO_DAILY_LIMIT_PRO = int(os.getenv("AI_VIDEO_DAILY_LIMIT_PRO", "30"))

# Цены в звёздах и что за них дают.
#
# 150 звёзд — это ~300 ₽ по цене покупки и ~$1.3–1.9 нам на руки после комиссий
# Telegram. Месяц активного пользователя стоит нам заметно меньше (LLM_COSTS.md),
# так что подписка окупает свой расход даже в худшем сценарии с айфона.
PRO_PRICE_STARS = int(os.getenv("PRO_PRICE_STARS", "150"))
PRO_PERIOD_DAYS = int(os.getenv("PRO_PERIOD_DAYS", "30"))

# Разовый пак для тех, кто не хочет ничего продлевать: вопросы не сгорают по
# дате и тратятся только после того, как кончились бесплатные.
PACK_PRICE_STARS = int(os.getenv("PACK_PRICE_STARS", "50"))
PACK_QUESTIONS = int(os.getenv("PACK_QUESTIONS", "30"))

# Раньше этого числа закрытых тренировок платное не предлагается вообще:
# человек ещё не понял, за что платит, и предложение денег на этом месте
# читается как «бот оказался платным». Упереться в бесплатный месячный лимит с
# такой историей практически невозможно, но если это случилось — упрёмся молча,
# без витрины.
PAYWALL_MIN_WORKOUTS = int(os.getenv("PAYWALL_MIN_WORKOUTS", "5"))
