...
async def finish_workout(request: Request) -> JSONResponse:
    ...
    # Тот же момент, что и начало (timeutil.backdated_moment): «дата started_at
    # + полдень UTC» у UTC+13/+14 давала конец на других сутках, чем начало.
    finished_at = workout["started_at"] if workout["status"] == "backfill" else None
