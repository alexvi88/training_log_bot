...
MUSCLE_GROUP_PRESETS = [
    ...
    ("Спина", "🔙", 5),
    ("Ноги", "🦵", 6),
    # «Кубики» — плитка шоколада и есть то, чем пресс называют в зале по обе
    # стороны языка («six-pack», «кубики»). До этой группы пресс жил в «Другое»
    # вместе с предплечьями и трапециями, и недельный объём его не видел вовсе
    # (см. formatting.VOLUME_HIDDEN_GROUPS).
    ("Пресс", "🍫", 7),
    ("Другое", "🔥", 8),
]

# Имя встроенной группы пресса — идентичность строки в базе (как и у
# остальных групп, навсегда по-русски). Отдельной константой, потому что на
...
# остальное), в отжиманиях узким хватом — столько же. В приседе без веса вверх
# едет всё, кроме голеней и стоп, — около 0.7; в обратных отжиманиях от скамьи
# ноги стоят на полу и забирают примерно половину.
#
# Списком, а не флагом у каждой строки каталога: EXERCISE_TEMPLATES — это пары
# (группа, имя), и раздувать их до кортежей ради девяти упражнений значило бы
# трогать каждую строку каталога.
#
...
EXERCISE_TEMPLATES = [
    ...
    ("Трицепс", "Обратные отжимания от скамьи"),
    # ---------- Бицепс ----------
    ...
    ("Спина", "Пулловер"),
    ("Спина", "Тяга с плинтов"),
    ("Спина", "Пулловер на верхнем блоке"),
    ("Спина", "Тяга двух гантелей в наклоне"),
    # ---------- Ноги ----------
    ...
    ("Ноги", "Гудморнинг"),
    ("Ноги", "Хип-траст со штангой"),
    ("Ноги", "Присед с гирей у груди"),
    ("Ноги", "Приседания без веса"),
    ("Ноги", "Выпады в ходьбе со штангой"),
    ("Ноги", "Сгибание ног стоя"),
    ("Ноги", "Обратная гиперэкстензия"),
    ("Ноги", "Жим носками в тренажёре"),
    ("Ноги", "Свинги с гирей одной рукой"),
    # ---------- Пресс (прямая, косые, кор и антиротация) ----------
    ("Пресс", "Скручивания"),
    ...
    ("Пресс", "Планка"),
    ("Пресс", "Боковая планка"),
    ("Пресс", "Велосипедные скручивания"),
    ("Пресс", "Русские повороты"),
    ("Пресс", "Колесо для пресса"),
    ("Пресс", "Обратные скручивания"),
    ("Пресс", "Пресс в тренажёре"),
    ("Пресс", "Жим Палоффа"),
    ("Пресс", "Дровосек в кроссовере"),
    ("Пресс", "Наклоны в сторону с гантелью"),
    ("Пресс", "Мёртвый жук"),
    # ---------- Другое (предплечья, трапеции) ----------
...
def canonical_exercise_name(shown_name: str) -> Optional[str]:
    ...
    То же, что и у групп, только шаблонов сто двадцать: имя шаблона в базе русское
...
def localized_exercise_name(canonical_name: str, lang: str) -> str:
    ...
    checks the full catalog — but a user's own hand-typed exercise can reach here
...
WORKOUT_PROGRAMS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "следующей. Становая — один рабочий подход на пять: она тяжелее "
            "остального и ест много сил. Изоляции тут нет намеренно — весь "
            "смысл в том, чтобы понемногу добавлять вес на гриф каждую "
            "тренировку, пока добавляется."
        ),
        "days": [
            ("Сила — день A", [
    ...
    },
    {
        ...
        "description": (
            ...
            "Больше объёма на группу, чем в программе на всё тело, но "
            "восстановиться проще, чем в сплите на каждый день. Если вечеров "
            "всего два — гоняй те же два дня один раз за неделю."
        ),
        "days": [
            ("Верх", [
