...
Про память. Всё, что ты «запоминаешь» о человеке (дни в неделю, оборудование,
опыт, цель, ограничения), НЕ записывается само: под твоим ответом появляется
вопрос «запомнить это?», и пишет уже он. Поэтому:
- не пиши «запомнил», «записал», «зафиксировал» — пока это неправда;
- скажи прямо, что предлагаешь запомнить, одной фразой и человеческими словами
  («Предлагаю запомнить: 3 дня в неделю, полный зал»), чтобы кнопка внизу не
  требовала догадки;
- если человек говорит, что записанное устарело («я теперь хожу 4 раза»), просто
  вызови save_athlete_profile с новым значением — он подтвердит.
    ...
    },
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "присылай только узнанное в этом ходе, старые поля останутся. После "
                "вызова коротко скажи, что записал, и упомяни кнопку отката — человек "
                "должен знать, что ты о нём запомнил. В следующих разговорах то же "
                "самое не переспрашивай: профиль лежит в get_training_overview. "
                "Стереть его человек может в ⚙️ Настройки → 🧬 Обо мне."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
async def _save_athlete_profile(
    ...
    # Ничего не пишем: сначала спрашиваем. Раньше профиль правился молча, а под
    # ответом появлялась кнопка «↩️ Забыть из профиля: дни в неде…» — человек не
    # понимал ни что это, ни что вообще записали, и кнопка читалась как команда
    # боту. Спрашивать «можно записать?» честнее отката: отменяют то, чего не
    # просили, а тут просто не делаем, пока не сказали «да».
    #
    # Череды подтверждений, которой боялся прежний комментарий, не будет: сборка
    # программы зовёт инструмент несколько раз за ход, но все поля копятся в
    # одном ожидании (см. handlers.ai_trainer._pending_profile), и вопрос под
    # ответом выходит один.
    changed = ", ".join(PROFILE_LABELS.get(key, key) for key in fields)
    # «Не записывать: дни в неделю,…» читалось как команда боту, а обрезанный
    # хвост не давал понять, о чём вообще речь: кнопка стояла над «Забрать» и
    # выглядела частью предложения программы. Говорим, что она делает —
    # забывает только что записанное про тебя, — и называем это словом «профиль»,
    # тем же, что и экран ⚙️ Настройки → «Что тренер про тебя знает».
    return (
        {
            "saved": False,
            "fields": fields,
            "note": (
                "НЕ записано: под твоим ответом появится вопрос, запоминать это "
                "или нет. Скажи одной фразой, что именно предлагаешь запомнить, "
                "и не пиши «запомнил» — пока это неправда."
            ),
        },
        {"profile_pending": fields, "changed": changed},
    )
