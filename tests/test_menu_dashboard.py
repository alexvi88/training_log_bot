...
# ---------- меню — текст всегда, картинка раз в сутки (или по кнопке) ----------


def _menu_callback(user_id: int, data: str = "", message_id: int = 1):
    """Мок callback для _show_main_menu/menu_summary: bot.send_photo — путь
    InaccessibleMessage, message.answer_photo — обычный (см. _send_summary_photo)."""
    bot = MagicMock()
    bot.send_photo = AsyncMock(return_value=SimpleNamespace(message_id=message_id + 500))
    bot.send_message = AsyncMock(return_value=SimpleNamespace(message_id=message_id + 500))
    message = MagicMock()
    ...
    message.answer = AsyncMock(
        return_value=SimpleNamespace(chat=SimpleNamespace(id=user_id), message_id=message_id + 1)
    )
...
async def _menu_state(user_id: int) -> FSMContext:
    return FSMContext(storage=MemoryStorage(), key=StorageKey(bot_id=1, chat_id=user_id, user_id=user_id))


async def _with_history(db, user_id: int) -> None:
    workout_handlers._heatmap_cache.pop(user_id, None)
    bench = await _own(db, user_id, BENCH)
    await _session(db, user_id, dt.date.today(), [(bench, 100, 5, 3)])


async def test_first_menu_open_of_the_day_sends_the_photo_before_the_text(fresh_db, user_id):
    """Первый вход в меню за сутки: картинка — отдельным сообщением, а текст
    меню приходит следом (уже не редактируется на месте — фото легло ниже,
    так же как это делает настоящий Bot API через chat_bottom middleware)."""
    chat_bottom.reset()
    db = fresh_db
    await _with_history(db, user_id)
    callback = _menu_callback(user_id, "menu:history")
    chat_bottom.note_message(callback.message.chat.id, callback.message.message_id)

    async def _photo_sent(*args, **kwargs):
        chat_bottom.note_message(callback.message.chat.id, callback.message.message_id + 1)
        return SimpleNamespace(chat=SimpleNamespace(id=user_id), message_id=callback.message.message_id + 1)

    callback.message.answer_photo.side_effect = _photo_sent

    await workout_handlers._show_main_menu(callback, await _menu_state(user_id))
...
async def test_second_menu_open_the_same_day_only_edits_the_text(fresh_db, user_id):
    """Второй вход в тот же день: картинка уже показана (отметка в БД от
    первого входа) — только правим текст на месте, без нового сообщения и без
    фликера."""
    chat_bottom.reset()
    db = fresh_db
    await _with_history(db, user_id)
    state = await _menu_state(user_id)

    first = _menu_callback(user_id, "menu:history", message_id=1)
    chat_bottom.note_message(first.message.chat.id, first.message.message_id)
    await workout_handlers._show_main_menu(first, state)

    # Отметка «показано сегодня» — в БД (см. MENU_SUMMARY_SHOWN_KIND), а не в
    # chat_bottom, так что для второго входа переиспользовать состояние
    # трекера первого не нужно: заново ставим второй экран дном чата, как оно
    # и было бы на самом деле после первого показа.
    chat_bottom.reset()
    second = _menu_callback(user_id, "menu:settings", message_id=2)
    chat_bottom.note_message(second.message.chat.id, second.message.message_id)
    await workout_handlers._show_main_menu(second, state)

    second.message.answer_photo.assert_not_awaited()
    second.message.edit_text.assert_awaited_once()
    second.message.delete.assert_not_awaited()
    chat_bottom.reset()


async def test_summary_button_sends_the_photo_even_if_already_shown_today(fresh_db, user_id):
    """«📊 Сводка» обходит суточный гейт: показанная сегодня картинка не мешает
    прислать её ещё раз по явному запросу."""
    chat_bottom.reset()
    db = fresh_db
    await _with_history(db, user_id)
    state = await _menu_state(user_id)
    ...
    button = _menu_callback(user_id, "menu:summary", message_id=2)
    chat_bottom.note_message(button.message.chat.id, button.message.message_id)
    await workout_handlers.menu_summary(button, state)

    button.message.answer_photo.assert_awaited_once()
    button.answer.assert_awaited_once()
    chat_bottom.reset()
