...
вообще создаёт бота (см. `api_v1.py`, докстринг модуля). Тот же тупик уже
решён в game_server._send_trainer_message — короткоживущий `Bot(token=...)`
на одну отправку, сессия закрывается сразу после: событие редкое (человек не
шлёт отзывы каждую секунду), а держать процессу-исключение отдельный
долгоживущий Bot ради одного эндпоинта незачем. Тот же приём и здесь.
