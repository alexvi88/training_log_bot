...
#
# Названия — про труд, а не про место в иерархии: каждое говорит, что человек
# делает («Каменщик», «Работяга», «Станок»), и ни одно не подкалывает того, кто
# стоит ниже. Тренер из подвала подкалывает только за пропуски (TONE_OF_VOICE),
# а звание видно на сводке каждый день — насмешка в нём работала бы постоянно.
RANKS: list[Rank] = [
    Rank(0, "🚪", "Новичок", 0, 0, 0.0),
    Rank(1, "🧱", "Каменщик", 5, 5_000, 0.5),
