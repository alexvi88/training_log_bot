...
def _consent_page(
    ...
    return _page(
        ...
        f"{destination_block}"
        '<p class="muted">Читать может всё это. Писать — только вес и еду, теми же '
        "словами, что и в чате с тренером; поправить или убрать запись можно потом "
        "в самом боте. Переписка с AI-тренером наружу не уходит, программы отсюда "
        "не собрать.</p>"
        f"{error_block}"
