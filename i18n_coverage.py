...
# задача. Экранные тексты, которые читаются извне как модульные атрибуты
# (persistent_menu.py: `from handlers.ai_trainer import INTRO_TEXT, RESUME_TEXT`),
# идут через module __getattr__ (PEP 562, тот же приём, что у formatting.py и
# ai_limits.py) — с одной оговоркой: он честно чинит только чтение через
# `module.ATTR` в момент использования, а persistent_menu.py делает
# `from ... import` на верхнем уровне файла (один раз, при первом импорте
# процесса) — этого не касаться нам нельзя, так что нижняя клавиатура
# («AI-тренер») показывает интро/резюме на языке по умолчанию (ru), а вход
# через инлайн-кнопку (menu:ai) — на языке пользователя. Пулы placeholder-фраз
