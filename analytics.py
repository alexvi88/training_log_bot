...
#
# Названия описывают этап, а не место в иерархии: «Втянулся» — про то, что
# человек уже ходит, а не про то, что он пока хуже остальных. Ни одно не
# подкалывает того, кто стоит ниже: тренер из подвала подкалывает только за
# пропуски (TONE_OF_VOICE), а звание висит на сводке каждый день — насмешка в
# нём работала бы постоянно. Соседние ступени должны ещё и различаться на слух:
# «Новичок» и «Новобранец» читались как одно и то же звание.
#
# Порог по числу тренировок — это календарь, а не усилие: при трёх тренировках в
# неделю (верхнее требование лестницы) 400 тренировок — это два с половиной года,
# и последние две ступени переставали быть целью. Ступени расставлены так, чтобы
...
# Доля рабочего веса, ниже которой изменение e1RM — это дрожание, а не прогресс.
# Расчётный максимум пересчитывается из веса и повторов, поэтому один лишний
# повтор в подходе двигает его на пару килограммов и на следующей тренировке
# двигает обратно. Два процента — примерно эта величина: на 220-килограммовой
# тяге порог выходит 4.4 кг, на 60-килограммовом разгибании — 1.2.
E1RM_NOISE_FRACTION = 0.02

# Абсолютный пол порога: на лёгких движениях два процента вырождаются в
# десятые доли, а десятая доля килограмма ростом не является ни на каком весе.
E1RM_NOISE_FLOOR_KG = 1.0


def e1rm_noise_floor(series: Iterable[float]) -> float:
    """Насколько должен измениться e1RM, чтобы это считалось изменением.

    Считается от самой тяжёлой точки серии, а не от последней: если человек
    откатился с пика, порог должен остаться прежним, иначе откат сам себя и
    объявил бы новой нормой.
    """
    peak = max((abs(v) for v in series), default=0.0)
    return max(E1RM_NOISE_FLOOR_KG, peak * E1RM_NOISE_FRACTION)
