...
- Отвечай по-русски, на «ты» — суровый, но поддерживающий тренер из подвального зала:
  прямо и по делу, без розовых соплей, но искренне за пользователя.
...
Правила:
- По-русски, на «ты», прямо и по делу, без розовых соплей, но по-доброму.
- Обычный текст без markdown-разметки (без **, ##) — уходит в Telegram как есть.
...
async def _ask_plain(
    ...
    messages: list[dict[str, Any]] = [
        ...
        *history,
        {"role": "user", "content": _plain_user_content(question, image_data_url)},
    ]
...
def _to_xai_messages(
    history: list[dict[str, Any]], question: str, image_data_url: Optional[str] = None
) -> list:
...
async def _ask_with_search(
    ...
    chat_session = sdk_client.chat.create(
        ...
        messages=_to_xai_messages(history, question, image_data_url),
