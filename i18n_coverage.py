...
ALLOWED_CYRILLIC: dict[str, set[str]] = {
    ...
    # ai_limits.py: _KIND_TITLES и preview_text (ai_limits.preview_text) —
    # экран-предупреждение только для своих аккаунтов (ADMIN_ID/TEST_USER_ID,
    # см. config.limit_preview_ids и CLAUDE.md, «Сколько это стоит»), плюс
    # строки `Block.log`/сообщения logger — для разработчика в логе, атлет их
    # не видит ни при каких обстоятельствах.
    "ai_limits.py": {
        "вопросы тренеру",
        "живой поиск, личная квота",
        "живой поиск, общий потолок",
        "разбор видео",
        "разбор еды",
        "расход за сутки",
        "расход за сутки, стоп",
        "⚠️ СРАБОТАЛ ЛИМИТ: ",
        "За сутки набежало ~$",
        "Обычный атлет сейчас увидел бы:",
        "Обычный атлет ничего не увидел бы — ответ просто идёт без свежести.",
        "Жми «Понятно» — до конца суток этот лимит тебя пропускает.",
        " из ",
        " за сутки",
        ": за сутки ~$",
