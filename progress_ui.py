...
# Постоянная времени кривой percent = CAP * (1 - e^(-t/tau)): быстро в начале,
# всё медленнее к потолку.
#
# Было 5 — под ожидание в 18 секунд, но сборка программы идёт дольше (потолок
# запроса — config.AI_REQUEST_TIMEOUT_SECONDS, 90 секунд), и на живой сборке
# кривая упиралась в 93% секунд за двадцать, а весь чек-лист из четырёх пунктов
# проставлялся за первые шесть. Дальше экран просто замирал — ровно в тот
# момент, когда ждать оставалось дольше всего, и замерший счётчик читается как
# зависший бот.
#
# При tau=14 и семи этапах (handlers.ai_trainer.PROGRAM_PROGRESS_STAGES)
# галочки встают на 2, 5, 8, 12, 18 и 27 секунде — новая примерно каждые
# пять секунд на всём протяжении ожидания, а не пачкой в начале. Последний
# этап не закрывается никогда: percent доходит до потолка только в пределе,
# и галочка на нём — это и есть настоящее «готово» (см. run_progress).
GROWTH_TAU = 14.0
