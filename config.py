...
# короткий ролик модель видит не только быстрее, но и КРУПНЕЕ. Один подход на
# 10–15 секунд честно лучше тех же повторов, снятых с подходом к снаряду и
# паузами. Ниже 30 этот потолок пока не опускаем — подход из пяти повторов со
# съёмом занимает секунд двадцать пять, и резать его значило бы отказывать
# живым роликам ради выигрыша, который мы не замеряли. Замерять это надо на
# проде, а не в комментарии.
#
