...
# «суперсетов не было».
#
# Критерий намеренно слабый: достаточно, чтобы окна касались (`<=`/`>=`), и
# минимальной длины пересечения нет. Один подход, воткнутый между двумя
# подходами другого упражнения, — это уже чередование, а вот два упражнения,
# сделанных подряд, окнами не пересекаются и суперсетом не считаются.
_SET_RANGES_SQL = (
